...
angular.module('assetManagementApp').controller('AlarmsCtrl', function($rootScope, $scope, $window, $compile, persistData, dataTable, toastNotifier, logReport, $filter, $translate, $uibModal, $log, $q, Session, DTOptionsBuilder, DTColumnBuilder, CONFIG_DATA, AUTH_EVENTS, alarmsFactory, alertNotifier) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
      .withOption('ajax', {
        url: ((CONFIG_DATA.SERVICES.getAlarms.replace('{sensorType}', sessionInfo['sensorType'])).replace('{tenantID}', alarmsFactory.getAlarmsFilter().tenantID || 'all')).replace('{towerID}', alarmsFactory.getAlarmsFilter().towerID || 'all'),
        type: 'GET',
        headers: {
        ...
        dataType: "json",
        data: function(data) {
          var alarmsFilterData = alarmsFactory.getAlarmsFilter();
          data.fromDate = $filter('date')(alarmsFilterData.fromDate, 'yyyy-MM-dd');
          data.toDate = $filter('date')(alarmsFilterData.toDate, 'yyyy-MM-dd');
          console.error(data);
          return data;
        },
        converters: {
          "text json": function(result) {
            ...
            if(response.status === 200) {
              ...
              var data= [];
              angular.forEach(response.data.records, function(alarm, index) {
                alarm['id']=index;
                data.push(alarm);
              });
